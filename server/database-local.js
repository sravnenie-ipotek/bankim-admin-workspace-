...
// Create database connection for local testing
const dbPath = path.join(__dirname, 'bankim_local_test.db');
...
const createTestTable = () => {
  ...
  try {
    ...
    console.log('✅ Local SQLite test table "test_users" created successfully');
    ...
    if (count === 0) {
      ...
      const sampleUsers = [
        ['John Doe (Local)', 'john.local@bankim.com', 'admin', 'active'],
        ['Jane Smith (Local)', 'jane.local@bankim.com', 'manager', 'active'],
        ['Bob Johnson (Local)', 'bob.local@bankim.com', 'user', 'inactive'],
        ['Alice Brown (Local)', 'alice.local@bankim.com', 'user', 'active'],
        ['Charlie Wilson (Local)', 'charlie.local@bankim.com', 'manager', 'pending'],
        ['Local Test User', 'test.local@bankim.com', 'user', 'active']
      ];
      ...
      console.log(`✅ Inserted ${sampleUsers.length} sample users (LOCAL)`);
    }
    
  } catch (error) {
    console.error('❌ Error creating local test table:', error);
  }
};
