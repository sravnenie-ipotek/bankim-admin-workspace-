...
const MortgageDrill: React.FC = () => {
  ...
  const handleEditClick = (action: MortgageAction) => {
    ...
    if (action.component_type?.toLowerCase() === 'text' || 
      ...
      // For other types, navigate to general edit page
      console.log('➡️ Navigating to general edit page for type:', action.component_type);
            ...
            <div className="table-column" style={{ width: '126px' }}>
              ...
              {currentActions.map((action, index) => (
                <React.Fragment key={`type-${action.id}`}>
                  <div className="column-cell">
                    <div style={{ color: 'var(--white, white)', fontSize: '14px', fontFamily: 'Arimo', fontWeight: '500', lineHeight: '21px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={getComponentTypeDisplay(action.component_type)}>
                      {getComponentTypeDisplay(action.component_type)}
                    </div>
                  </div>
                  <div className="column-divider"></div>
                </React.Fragment>
