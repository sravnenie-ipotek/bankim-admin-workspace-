...
async function fixContentStructure() {
  const client = await contentPool.connect();
  
  try {
    await client.query('BEGIN');
    console.log('🔧 FIXING CONTENT STRUCTURE - DEDUPLICATION & PROPER TRILINGUAL');
    console.log('================================================================');
    
    // 1. ANALYZE CURRENT DUPLICATION PATTERN
    console.log('\n📊 1. ANALYZING CURRENT DUPLICATION PATTERN');
    console.log('------------------------------------------');
    ...
    let totalTranslationsConsolidated = 0;
    
    // 2. CONSOLIDATE DUPLICATES WITH PROPER TRILINGUAL STRUCTURE
    console.log('\n🔄 2. CONSOLIDATING DUPLICATES WITH TRILINGUAL STRUCTURE');
    console.log('-------------------------------------------------------');
    ...
    for (const duplicate of duplicatesResult.rows) {
      ...
      console.log(`\n📝 Processing: ${duplicate.content_key}`);
      console.log(`   Keep ID: ${keepId}, Remove IDs: [${removeIds.join(', ')}]`);
      
      // Get all translations from duplicate items
      ...
      console.log(`   Found languages: ${Object.keys(translationsByLang).join(', ')}`);
      
      // Delete all translations for duplicate items
      await client.query(
        'DELETE FROM content_translations WHERE content_item_id = ANY($1)',
        [allIds]
      );
      
      // Insert consolidated translations for the kept item
      ...
      console.log(`   ✅ Consolidated to 1 item with 3 translations`);
    }
    
    console.log(`\n🗑️ Removed ${totalDuplicatesRemoved} duplicate content items`);
    console.log(`🌐 Consolidated ${totalTranslationsConsolidated} translations`);
    ...
    // 4. EXPECTED COUNTS VERIFICATION
    console.log('\n📋 4. EXPECTED COUNTS VERIFICATION');
    console.log('---------------------------------');
    ...
    console.log('\n🎉 CONTENT STRUCTURE FIX COMPLETE!');
    console.log('==================================');
    console.log(`✅ Removed ${totalDuplicatesRemoved} duplicate content items`);
    console.log(`✅ Consolidated ${totalTranslationsConsolidated} translations`);
    console.log(`✅ Final structure: ${totalItems} items, ${totalTranslations} translations`);
    console.log(`✅ Expected structure match: ${allMatched ? 'PERFECT' : 'NEEDS ADJUSTMENT'}`);
    console.log(`✅ Trilingual completeness: ${totalTranslations === totalExpected ? 'PERFECT' : 'INCOMPLETE'}`);
    console.log('🚀 Database now properly structured for admin panel!');
    
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ Error fixing content structure:', error);
    throw error;
  } finally {
    client.release();
  }
...
if (require.main === module) {
  fixContentStructure()
    .then(() => console.log('🎉 Content structure fix completed!'))
    .catch(error => console.error('💥 Content structure fix failed:', error));
}
