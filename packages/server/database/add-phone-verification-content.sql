...
-- Now insert translations for each content item
-- We'll use a CTE to get the IDs of the content items we just inserted/updated
WITH content_ids AS (
  SELECT id, content_key 
  FROM content_items 
  WHERE screen_location = 'mortgage_phone'
)
INSERT INTO content_translations (content_item_id, language_code, title_ru, title_he, title_en, created_at, updated_at)
SELECT 
  ci.id,
  'multi', -- Using 'multi' to indicate all languages in one row
  CASE ci.content_key
...
-- Insert translations for code verification screen
WITH code_content_ids AS (
  SELECT id, content_key 
  FROM content_items 
  WHERE screen_location = 'mortgage_code_input'
)
INSERT INTO content_translations (content_item_id, language_code, title_ru, title_he, title_en, created_at, updated_at)
SELECT 
  ci.id,
