/**
 * CreditRefiTextEdit Component
 * Edit page for text-type credit refinancing content items
...
const CreditRefiTextEdit: React.FC = () => {
  ...
  const fetchContentData = async () => {
    try {
      ...
      console.log(`📋 Fetching credit-refi content item with ID: ${actionId}`);
      ...
      console.error('Error fetching credit-refi content:', err);
      setError('Failed to load credit-refi content data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (data: { ruText: string; heText: string; additionalTexts: Array<{ ru: string; he: string }> }) => {
    ...
    try {
      ...
      console.log(`💾 Saving credit-refi content item ${content.id}:`, updateData);
      const response = await apiService.updateContentItem(content.id, updateData);
      
      if (response.success) {
        console.log('✅ Credit-refi content updated successfully');
        handleCancel(); // Go back to previous page
      } else {
        setError('Failed to save credit-refi content');
      }
    } catch (err) {
      console.error('Error saving credit-refi content:', err);
      setError('Failed to save credit-refi content');
    } finally {
      setSaving(false);
    }
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    { 
      label: 'Контент сайта', 
      onClick: () => navigate('/content/credit-refi'),
      isActive: false 
    },
    { 
  ...
  if (loading || !content) {
    return (
      <SharedTextEdit
        ...
        pageSubtitle="Редактирование текста рефинансирования кредита"
      />
    );
  }
...
export default CreditRefiTextEdit; 